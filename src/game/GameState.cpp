...
void GameState::InitializeUI() {
    ...
    // Initialize decoupled GUI layer (views own rendering logic)
    if (!m_GuiLayer) {
...
void GameState::RenderStationsContent() {
    // Delegate rendering to the StationView owned by GuiLayer
    if (m_GuiLayer && m_GuiLayer->GetStationView()) {
        m_GuiLayer->GetStationView()->Render(this, nullptr);
    }
}
...
void GameState::RenderUI(Renderer* renderer) {
    ...
    // Delegate navigation bar rendering to the NavigationView owned by GuiLayer
    if (m_GuiLayer && m_GuiLayer->GetNavigationView()) {
        m_GuiLayer->GetNavigationView()->Render(this, nullptr);
    }
}

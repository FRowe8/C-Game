...
void ParticleCollection::Initialize() {
    ...
    Loggerger::Info("ParticleCollection initialized with 28 particles");
}
...
void ParticleCollection::DiscoverParticle(ParticleType type) {
    ...
    if (!particle->discovered) {
        ...
        Loggerger::Infof("*** NEW PARTICLE DISCOVERED: ", particle->name, " (",
                   static_cast<i32>(particle->rarity), ") ***");
    } else {
        particle->count++;
        Loggerger::Infof("Found another ", particle->name, "! Total: ", particle->count);
    }
}
...
void ParticleCollection::EquipParticle(ParticleType type) {
    ...
    if (m_EquippedParticles.size() >= 3) {
        Loggerger::Warn("Cannot equip more than 3 particles! Unequip one first.");
        return;
    }
    ...
    if (p) {
        Loggerger::Infof("Equipped: ", p->name);
    }
}
...
void ParticleCollection::UnequipParticle(ParticleType type) {
    ...
    if (it != m_EquippedParticles.end()) {
        ...
        Particle* p = GetParticle(type);
        if (p) {
            Loggerger::Infof("Unequipped: ", p->name);
        }
    }
}
